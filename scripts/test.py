"""Testing script for the model. Use through main.py."""
...
def test(cfg: OmegaConf):
    ...
    try:
        ...
        assert Path(cfg.trainer.ckpt_path).exists()
        trainer.test(
            model=model, datamodule=datamodule, ckpt_path=cfg.trainer.ckpt_path
        )
