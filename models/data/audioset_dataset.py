...
EPS = 0.01  # this is pretty large but works the best...
...
class AudioSetDataset(Dataset):
    def __init__(
        ...
        self.split_file_path = split_dir_path / f"audioset_{self.split}.txt"
        assert (
        ...
        if label_meta_path is None:
            label_meta_path = meta_path.parent / "audioset_labels.csv"

        assert (
            label_meta_path.is_file()
        ), f"Label meta file {label_meta_path.as_posix()} does not exist."
        ...
        with open(self.split_file_path, "r", encoding="utf-8") as f:
            clip_paths_list = f.read().splitlines()
        clip_paths = set(sorted(clip_paths_list))

        if excluded_files_path is not None:
            excluded = self._get_bad_examples(excluded_files_path)
            clip_paths = clip_paths.difference(excluded)

        if included_files_path is not None:
            included = self._get_good_examples(included_files_path)
            clip_paths = clip_paths.intersection(included)
        ...
        self.dataset = [self.data_path / f"{c}.mp4" for c in clip_paths]
    ...
    def __getitem__(self, idx: int) -> dict:
        if self.split == "train":
            ...
            start_pts = self.fixed_start_pts.get(self.dataset[idx].stem, 6.66)
