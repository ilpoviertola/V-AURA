...
https://github.com/facebookresearch/audiocraft/blob/main/audiocraft/modules/codebooks_patterns.py.
...
if __name__ == "__main__":
    nq = 2
    cb = torch.range(0, 9).expand(nq, -1).unsqueeze(0)
    dpp = DelayedPatternProvider(n_q=nq)
    p = dpp.get_pattern(cb.shape[-1])
    sequence_codes, _, _ = p.build_pattern_sequence(
        cb[:, :, :-1].contiguous(),
        cb.shape[-1],
        keep_only_valid_steps=False,
    )
    print(sequence_codes)
    print()
    sequence_codes_reverted = p.revert_pattern_sequence(
        sequence_codes, cb.shape[-1], False
    )
    print(sequence_codes_reverted)
    print()

    # Create a tensor with shape (1, 2, sequence_len, 25)
    tensor_shape = (1, 2, sequence_codes.shape[-1], 25)
    logits = torch.ones(tensor_shape)
    # Fill the tensor with values matching the row number
    for row in range(tensor_shape[-2]):
        logits[:, :, row, :] *= row

    print(logits)
    logits = logits.permute(0, 3, 1, 2)
    logits_reverted, v, idxs = p.revert_pattern_logits(logits, cb.shape[-1], False)
    logits_reverted = logits_reverted.permute(0, 2, 3, 1)
    print(logits_reverted)
    print()
